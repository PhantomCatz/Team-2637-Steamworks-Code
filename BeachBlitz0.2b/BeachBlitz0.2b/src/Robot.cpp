...
public:
	...
	void gearSequence(bool closeFlaps)
	{
		...
		if(gearMechOpen==false)
			toggleGearMech();
		Wait(.2);                 //original wait time 0.5  DD 10-7-17
		kick();
		if(closeFlaps == true){
			toggleGearMech();
		}
	...
	void kick()
	{
		...
		while (hardware->kickTimer->Get() < 0.25) //0.4
		{
		...
		{
			hardware->kicker->Set(-0.5);       //original speed -0.4  DD 10-7-17
			driver->setDriveControl(xboxDrive);
		}
	...
	void EncoderStraightDrive(double speed, double distance, double sampleTime,double timeout,bool useGearButton)
	{
		...
		while(fabs(hardware->wheelEncoder->GetDistance()) < distance && gearState==true && done != true)
		{
			...
			/*if((fabs(hardware->wheelEncoder->GetDistance()) - distance) < 10 && speed<0)
				driver->AutoArcade(.4, straightkP*currentAngle + straightkD*derivative);
			else if((fabs(hardware->wheelEncoder->GetDistance()) - distance) < 10)
				driver->AutoArcade(.4, straightkP*currentAngle + straightkD*derivative);
			else*/
	...
	void EncoderNoBrake(double speed, double distance, double sampleTime,double timeout,bool useGearButton)
		{
			int loopCount = 0;
			double encoderIssues=0;
			int dbgCount1=0;
	...
	void middleAuto(int allianceColor)
	{
		...
		{
			wiggle(); //added wiggle DD 10-7-17
			EncoderNoBrake(.5,5,T_RESET,1,USE_GEAR_BUTTON); //original distance 6 + original timeout 3.1
		}
		gearSequence(false);
	...
	void outsideGearAuto(int allianceColor, bool alwaysKick)
	{
		...
		EncoderNoBrake(.5,60.0,T_RESET,3,USE_GEAR_BUTTON);
		...
		{
			//wiggle();
			EncoderNoBrake(.45,4,T_RESET,0.8,USE_GEAR_BUTTON);			//If gear button is not pressed then wiggle and drive forward
		}
		if(hardware->gearButton->Get()==false)
		{
			EncoderStraightDrive(-.2,2,T_RESET,0.3,USE_GEAR_BUTTON);			//if the gear button is pressed then drive backward a little
			gearSequence(false);
			EncoderStraightDrive(-.6,40.0,T_RESET,3,DONT_USE_GEAR_BUTTON);        //back up
			if(allianceColor == BLUE_ALLIANCE)
				PDTurn(50,2.5);
			else																//turn towards neutral zone
				PDTurn(-50,2.5);
			EncoderStraightDrive(.7,380.0,T_RESET,9,DONT_USE_GEAR_BUTTON);     //drive into neutral zone previously .7,250,3.7 AL
		}
		else                                                                  //if gear button is still not pressed
		{
			...
			if(alwaysKick)                                                      //kick if we have a ground gear mech
			{
				...
				if(allianceColor == BLUE_ALLIANCE)
					...
					PDTurn(-60,2.5);
				EncoderStraightDrive(1,380.0,T_RESET,9,DONT_USE_GEAR_BUTTON);     //drive into neutral zone
			}
		}
		if(gearMechOpen)
			toggleGearMech();
	...
	void boilerAuto1(int allianceColor, bool alwaysKick)
	{
		...
		EncoderStraightDrive(.6,47.0,T_RESET,3,false);     //Match 7: used 37 in and 1.4 sec (4/21/17 11:16) AL


		if(allianceColor == BLUE_ALLIANCE)
			PDTurn(85,3);                          //Match 7: used 90 degree turn (4/21/17 11:26) AL
	...
	void TeleopPeriodic()
	{
		...
		if(joystickTest->GetRawButton(1)==1)
			EncoderStraightDrive(.7, 100, 5.0,10,true); //speed 10 distance 100 in, 5ms update time,5 sec timeout
